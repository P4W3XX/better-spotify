...
const TopBar = ({
  ...
  return (
    <div
      style={{
        opacity: scrollY > 0 ? (scrollY * 0.3) / 100 : 0,
      }}
      className={` w-full md:h-[7rem] h-[4.5rem] border-b md:p-4 p-2 flex justify-between items-center  bg-[#3c2428] z-20 top-0 fixed ${
    ...
    >
      <div className=" flex items-center space-x-4">
        <Image
          ...
          width={500}
          height={500}
          className="rounded-lg cursor-pointer md:size-[5rem] size-[3.5rem] shadow-[0_0_20px_0_rgba(0,0,0,0.5)] shadow-black/60"
        />
        <div>
          <h1 className=" md:text-3xl text-2xl font-semibold">4X4</h1>
          <p className=" text-white/50 md:text-sm text-xs font-medium">
            Travis Scott
          </p>
        </div>
      </div>
      <button className=" hover:scale-105 active:scale-95 transition-all cursor-pointer md:size-[4rem] size-[2.5rem] bg-white rounded-full flex items-center justify-center">
        <Play className=" text-black md:size-[24px] size-[16px]" fill="black" />
      </button>
    </div>
  );
};
...
export default function Album() {
  ...
  return (
    <div
      ...
      className=" relative w-full h-svh flex flex-col overflow-auto"
      ref={handleRef}
    >
      ...
      <div className=" md:p-7 p-4 pt-8 md:pt-7 flex flex-col md:flex-row items-center md:items-end space-y-4 md:space-y-0 relative md:space-x-8 z-10">
        ...
        <Image
          ...
          width={500}
          height={500}
          style={{ scale: scrollY > 0 ? Math.max(0, 1 - scrollY / 900) : 1 }}
      ...
      <div className=" w-full h-fit space-y-3 flex flex-col bg-black/60">
        <div
          className={`w-full hidden  md:flex flex-col space-y-2 items-center sticky md:px-7 px-4 top-[7rem] z-10`}
          style={{
        ...
        <div className=" flex flex-col space-y-3 md:px-7 px-2">
          <SongPreview index={0} />
          <SongPreview index={1} />
          <SongPreview index={1} />
          <SongPreview index={1} />
